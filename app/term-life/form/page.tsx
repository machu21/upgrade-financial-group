...
import { supabase } from "@/lib/supabase"; // Adjust this based on your setup

export default function TermLifeQuiz() {
  ...
  const isStep4Valid = formData.termLength !== "";
  const isStep5Valid =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Resolve final numerical values between sliders and custom inputs
    const finalPolicyAmount = formData.policyCustom 
      ? parseInt(formData.policyCustom.replace(/\D/g, "")) || formData.policyAmount 
      : formData.policyAmount;

    const finalMonthlyPremium = formData.premiumCustom 
      ? parseInt(formData.premiumCustom.replace(/\D/g, "")) || formData.monthlyPremium 
      : formData.monthlyPremium;

    try {
      const { error } = await supabase
        .from('term_life_requests')
        .insert([
          {
            living_benefits: formData.livingBenefits,
            policy_amount: finalPolicyAmount,
  ...
  const termOptions = [
    ...
    { label: "20–30 Years", value: "20-30 years" },
  ];
        ...
        {!isSuccess && (
          <div className="text-center mb-10">
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white mb-4">
              Design Your <span className="text-primary">Term Life Plan</span>
            </h1>
            <p className="text-white/80 font-medium">
              Answer a few quick questions to customize your Term Life policy.
            </p>
          </div>
        )}
            ...
            {!isSuccess ? (
                      ...
                      {["With Living Benefits", "Without Living Benefits"].map(
                        (option) => (
                          <button
                            key={option}
                            onClick={() =>
                              setFormData({ ...formData, livingBenefits: option })
                            }
                    ...
                    <div className="mt-4 space-y-6">
                      ...
                      <div className="relative pt-4">
                        <div
                          ...
                          style={{
                            width: `${
                              ((formData.policyAmount - 50000) / (1000000 - 50000)) *
                              100
                            }%`,
                          }}
                        />
                    ...
                    <div className="mt-4 space-y-6">
                      ...
                      <div className="relative pt-4">
                        <div
                          ...
                          style={{
                            width: `${
                              ((formData.monthlyPremium - 50) / (5000 - 50)) * 100
                            }%`,
                          }}
                        />
                        <input
                ...
                {step === 4 && (
                  <div className="animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col h-full">
                    <div className="flex items-center gap-3 mb-8">
                      ...
                      <h2 className="text-2xl font-bold text-slate-900">
                        How long are you looking to pay?
                      </h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 mt-4">
                      {termOptions.map((opt) => (
                        <button
                          key={opt.value}
                          onClick={() =>
                            setFormData({ ...formData, termLength: opt.value })
                          }
                          className={`p-6 rounded-2xl border-2 text-lg font-bold transition-all text-left flex items-center justify-between ${
                            formData.termLength === opt.value
                              ? "bg-primary/5 border-primary text-primary shadow-sm"
                              : "bg-white text-slate-600 border-slate-200 hover:border-slate-300 hover:bg-slate-50"
                          }`}
                        >
              ...
              <div className="flex flex-col items-center justify-center h-full text-center animate-in zoom-in-95 fade-in duration-700 py-12 my-auto">
                ...
                <p className="text-lg text-slate-600 max-w-sm mx-auto leading-relaxed">
                  ...
                  ! An advisor will review your Term Life preferences and contact
                  you shortly with personalized options.
                </p>
              </div>
            )}
          </div>
